...
function SearchContent({ videos }: { videos: CatalogVideo[] }) {
  const params = useSearchParams();
  const query = params.get("q") || "";
  const genre = params.get("genre") || "All";
  const type = params.get("type") || "All";

  const filtered = filterVideos(videos, { query, genre, type });
...
export default function SearchClient({ videos }: { videos: CatalogVideo[] }) {
  return (
